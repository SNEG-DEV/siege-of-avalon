...
const
  ...
  MFP_PLAYBACK_ENDED = 1;
  MFP_PLAYBACK_ERROR = 2;
  MFP_PLAYBACK_NO_VIDEO = 3;
...
begin
  ...
  if Failed(hr) then
  begin
    PostMessage(m_hwndEvent, WM_APP_PLAYER_EVENT, MFP_PLAYBACK_ERROR, hr);
  end;
end;
...
begin
  ...
  PostMessage(m_hwndEvent, WM_APP_PLAYER_EVENT, MFP_PLAYBACK_ENDED, 0);
  Result := S_OK;
end;
...
begin
  ...
  if Failed(m_pVideoDisplay.GetNativeVideoSize(a, b)) then
  begin
    PostMessage(m_hwndEvent, WM_APP_PLAYER_EVENT, MFP_PLAYBACK_NO_VIDEO, 0);
  end;
  Result := S_OK;
end;
